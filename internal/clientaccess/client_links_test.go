...
func TestBuildClientLinksUsesClientProfilesWhenPresent(t *testing.T) {
	...
	if byName["naive/alice"].URI != "naive+https://alice:alice-pass@vpn.example.com" {
		t.Fatalf("naive alice URI = %q", byName["naive/alice"].URI)
	}
	if byName["naive/bob"].URI != "naive+https://bob:bob-pass@vpn.example.com" {
...
func TestBuildClientLinksUsesPerInboundPasswordAndGlobalFallback(t *testing.T) {
	...
	if byName["naive"].URI != "naive+https://veil:global-naive@vpn.example.com" {
