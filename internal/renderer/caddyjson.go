...
func renderServer(key bindregistry.BindKey, owner caddyassembly.CaddyBindOwner, caps caddycapabilities.CaddyCapabilities) (map[string]any, error) {
	...
	case caddyassembly.CaddyOwnerPanel:
		if owner.BackendPort < 1 || owner.BackendPort > 65535 {
			return nil, fmt.Errorf("panelListen must be host:port")
		}
		if owner.WebBasePath == "" || owner.WebBasePath == "/" {
			return nil, fmt.Errorf("webBasePath is required for caddy Panel access")
		}
