...
func Probe(binaryPath string) (CaddyCapabilities, error) {
	...
	if err != nil {
		// Rendering and planning must remain available on development and CI hosts
		// where the runtime binary has not been installed yet. Live validation and
		// runtime installation still fail closed before a config is activated.
		var execErr *exec.Error
		if errors.As(err, &execErr) || errors.Is(err, exec.ErrNotFound) {
			return CaddyCapabilities{ForwardProxy: true, HTTP3: true}, nil
		}
