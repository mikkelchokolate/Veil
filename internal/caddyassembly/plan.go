...
type CaddyBindOwner struct {
	...
	Upstream     string           // Optional outbound proxy for naive traffic
}
...
func BuildRenderPlan(
	...
	if settings.PanelAccess == "caddy" {
		...
		key := bindregistry.BindKey{Address: "0.0.0.0", Port: publicPort, Network: bindregistry.ListenTCP}
		...
		if err := setServer(servers, key, CaddyBindOwner{
			Kind: CaddyOwnerPanel, Domain: panelDomain, BackendPort: backendPort, WebBasePath: webBasePath,
		}); err != nil {
			return CaddyRenderPlan{}, nil, err
		}
	}
	...
	for _, inb := range inbounds {
		...
		if port < 1 || port > 65535 {
			return CaddyRenderPlan{}, nil, fmt.Errorf("naive inbound %q publicPort must be between 1 and 65535", inb.Name)
		}
...
func naiveUsers(inbound model.Inbound, settings model.Settings) []CaddyNaiveUser {
	...
	for _, p := range inbound.Profiles {
		...
		users = append(users, CaddyNaiveUser{Username: username, Password: p.Password})
	}
	if len(users) > 0 {
		return users
	}
