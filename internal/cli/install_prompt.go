...
func (p InstallPrompt) PromptMissingOptions(domain *string, email *string, sharedPort *int, panelPort *int) error {
	...
	if *sharedPort == 0 {
		for {
			fmt.Fprint(p.out, "Shared proxy port: ")
			value, err := reader.ReadString('\n')
			if err != nil {
				return err
			}
...
func promptInstallOptions(cmd *cobra.Command, domain *string, email *string, sharedPort *int, panelPort *int) error {
	return NewInstallPrompt(cmd.InOrStdin(), cmd.OutOrStdout()).PromptMissingOptions(domain, email, sharedPort, panelPort)
}
