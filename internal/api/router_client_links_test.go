...
func TestClientLinksEndpointBuildsEnabledProxyLinks(t *testing.T) {
	...
	if links["naive"].Protocol != "naiveproxy" || links["naive"].Transport != "tcp" || links["naive"].Port != 443 || links["naive"].URI != "naive+https://veil:naive-secret@vpn.example.com" {
...
func TestClientLinksSubscriptionEndpointRejectsUnknownQueryBeforeConfigValidation(t *testing.T) {
	...
	if message := responseErrorMessage(t, w.Body.Bytes()); !strings.Contains(message, `unsupported subscription query "offset"`) {
