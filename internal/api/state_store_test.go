...
func TestStateStoreEncryptsAndDecryptsInboundPasswords(t *testing.T) {
	...
	snapshot := managementSnapshot{
		...
		Inbounds: []Inbound{{Name: "hy2-vip", Protocol: "hysteria2", Transport: "udp", Port: 8443, Enabled: true, Password: "vip-secret"}},
	}
	...
	if strings.Contains(string(raw), "vip-secret") {
		t.Fatalf("raw state leaked inbound password: %s", string(raw))
	}
