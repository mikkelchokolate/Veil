...
- `system` runs in a booted systemd smolvm guest: privilege-boundary, e2e, and
  ...
  first-inbound API coverage for the direct-mode protocols — hysteria2,
  mieru, olcRTC; NaiveProxy data-path coverage is owned by e2e — certificate
  issuance against a controlled pebble ACME CA, a caddy-mode public-route
  leg, and a guest reboot → post-reboot persistence phase when the smolvm
  backend stages the run). On GitHub Actions the same job runs on
  ubuntu-24.04 amd64 and arm64 hosted runners with live systemd.
