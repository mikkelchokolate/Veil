...
- Multiple enabled **Inbounds** of the same protocol can produce multiple **Client links**. Hysteria2 and olcRTC are rendered as isolated per-Inbound Generated config artifacts and managed through systemd template units. NaiveProxy **Inbounds** consolidate into a single `caddy/config.json` served by one `veil-caddy.service`. Mieru **Inbounds** aggregate into one **Generated config set** so TCP and UDP **Transport bindings** can share a numeric port.
