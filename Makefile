...
ci-job:
	@test -n "$(JOB)" || (echo "usage: make ci-job JOB=<frontend|test|lint|browser-e2e|privilege-boundary|e2e|install-acceptance|package-smoke|image-build|multi-process|sigkill|filesystem-faults|full|stress>" >&2; exit 1)
