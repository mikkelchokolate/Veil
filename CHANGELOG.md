...
- Legacy protocol stack and shared proxy port install inputs are hidden or ignored for compatibility; dead install-time protocol artifact, stack policy, shared port planning, port availability, protocol binary acquisition, and Naive Caddy build Modules were removed from the installer.
