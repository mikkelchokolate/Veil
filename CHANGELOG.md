...
- RURecommendedProfile, Veil install/repair workflow inputs, the Settings Interface, RU-recommended profile preview Interface, Panel settings actions, Client link responses, and the curl installer parser no longer carry legacy protocol stack/install fields; Settings JSON still accepts legacy stack behind a compatibility Module without re-emitting it.
