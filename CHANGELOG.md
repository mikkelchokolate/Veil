...
- RURecommendedProfile, Veil install/repair workflow inputs, RU-recommended profile preview Interface, Panel settings actions, Client link responses, and the curl installer parser no longer carry legacy protocol stack/install fields; unused legacy stack normalization helpers were removed, and the remaining legacy stack compatibility Modules are Settings normalization and hidden CLI/script flag rejection.
